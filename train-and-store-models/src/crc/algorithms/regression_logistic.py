...
class RegressionLogistic:
    ...
    def execute(self, task: TaskCreateModel) -> int:
        # Cargar datos numéricos con variable objetivo binaria
        X, y, feature_ids = SourceBuilder.build_numeric_data_with_binary_target(task.id_source)
        ...
        # Obtener coeficientes, errores estándar, z-values y p-values
        coeffs = model.params
        std_errs = model.bse
        z_values = model.tvalues  # z en lugar de t
        ...
        intercept = Regression(
            ...
            value=float(z_values[0]),
            p_value=float(p_values[0])
        )
        self.regression_repo.add(intercept)

        # Guardar los coeficientes asociados a las variables predictoras
